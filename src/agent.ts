...
// Import analysis functions for /init command
import { detectProjectType, extractConventions, getProjectOverview } from './analysis/index.js';
...
export class Agent {
  ...
  exit      - Quit the application`;
                case '/init':
                    try {
                        // Call analysis functions directly
                        const projectTypeInfo = detectProjectType('.');
                        const conventionsInfo = await extractConventions('.');
                        const overviewInfo = getProjectOverview('.');

                        // Generate CRAFT.md content
                        const craftContent = `# CRAFT.md - CodeCraft Project Analysis

## Project Overview
${JSON.stringify(overviewInfo, null, 2)}

## Technology Stack
${JSON.stringify(projectTypeInfo, null, 2)}

## Code Conventions
${JSON.stringify(conventionsInfo, null, 2)}
