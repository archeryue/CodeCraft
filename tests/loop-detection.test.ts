...
describe('Loop Detection', () => {
    ...
    describe('Happy Path Tests', () => {
        it('should detect when same tool is called 3+ times consecutively', () => {
            recovery.recordAction({ tool: 'ReadFile', params: { path: 'foo.ts' } });
            recovery.recordAction({ tool: 'ReadFile', params: { path: 'foo.ts' } });
            recovery.recordAction({ tool: 'ReadFile', params: { path: 'foo.ts' } });
        ...
        it('should detect repetition loops (A-B-A-B-A-B pattern)', () => {
            recovery.recordAction({ tool: 'SearchCode', params: { query: 'foo' } });
            recovery.recordAction({ tool: 'ReadFile', params: { path: 'bar.ts' } });
            recovery.recordAction({ tool: 'SearchCode', params: { query: 'foo' } });
            recovery.recordAction({ tool: 'ReadFile', params: { path: 'bar.ts' } });
            recovery.recordAction({ tool: 'SearchCode', params: { query: 'foo' } });
        ...
        it('should detect when same file is read 3+ times', () => {
            recovery.recordAction({ tool: 'ReadFile', params: { path: 'src/agent.ts' } });
            recovery.recordAction({ tool: 'ReadFile', params: { path: 'src/agent.ts' } });
            recovery.recordAction({ tool: 'ReadFile', params: { path: 'src/agent.ts' } });
        ...
        it('should allow legitimate repeated calls (different parameters)', () => {
            recovery.recordAction({ tool: 'ReadFile', params: { path: 'file1.ts' } });
            recovery.recordAction({ tool: 'ReadFile', params: { path: 'file2.ts' } });
            recovery.recordAction({ tool: 'ReadFile', params: { path: 'file3.ts' } });
    ...
    describe('Edge Cases', () => {
        it('should NOT flag legitimate patterns (read → edit → verify flow)', () => {
            recovery.recordAction({ tool: 'ReadFile', params: { path: 'foo.ts' } });
            recovery.recordAction({ tool: 'EditFile', params: { path: 'foo.ts', old_string: 'a', new_string: 'b' } });
            recovery.recordAction({ tool: 'ReadFile', params: { path: 'foo.ts' } });
        ...
        it('should detect similar parameters (not just exact matches)', () => {
            const actions = [
                { tool: 'ReadFile', params: { path: 'foo.ts', offset: 10, limit: 20 } },
                { tool: 'ReadFile', params: { path: 'foo.ts', offset: 30, limit: 20 } },
                { tool: 'ReadFile', params: { path: 'foo.ts', offset: 50, limit: 20 } }
            ];
    ...
    describe('Detection Patterns', () => {
        ...
        it('Alternating pattern: A-B-A-B-A (2+ cycles)', () => {
            recovery.recordAction({ tool: 'ListDirectory', params: { path: 'src' } });
    ...
    describe('Output', () => {
        it('Return loop type: "consecutive" | "alternating" | "parameter_similarity"', () => {
            ...
            const actions = [
                { tool: 'ReadFile', params: { path: 'foo.ts', offset: 1 } },
                { tool: 'ReadFile', params: { path: 'foo.ts', offset: 2 } },
                { tool: 'ReadFile', params: { path: 'foo.ts', offset: 3 } }
            ];
            expect(similarityDetector.getSimilarityType(actions)).toBe('same_file_different_offsets');
        });
