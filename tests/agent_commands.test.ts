...
vi.mock('../src/tools/extract_conventions', () => ({
  extractConventionsTool: {
    execute: vi.fn()
  }
}));

vi.mock('../src/tools/get_project_overview', () => ({
...
describe('Agent /init Command', () => {
  ...
  beforeEach(async () => {
    ...
    // Get mocked tools
    const { detectProjectTypeTool } = await import('../src/tools/detect_project_type');
    const { extractConventionsTool } = await import('../src/tools/extract_conventions');
    const { getProjectOverviewTool } = await import('../src/tools/get_project_overview');
